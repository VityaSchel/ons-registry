...
export function Input({ mode, placeholder, onChange, maxLength }: {
  ...
  const easterEggAvailable = React.useMemo(() => {
    return !['devio', 'vitya-devio', 'devio-vitya', 'vitya+devio', 'devio+vitya', 'vitya.plus.devio.love', 'vitya+dima', 'dima+vitya'].includes(value)
  }, [value])

  return (
    <>
      <span className='absolute left-8 top-6 transition-opacity duration-150 pointer-events-none text-ellipsis overflow-hidden max-w-full line-clamp-1 break-all font-open-sans' style={{
        width: 'calc(100% - 4rem)',
      ...
      <span className={cx(`absolute mt-0.5 ml-0.5 left-8 top-6 text-4xl ${openSans.className} overflow-hidden max-w-full break-all text-clip h-10 transition-opacity`, {
      ...
      <input
        type="text"
        className={cx(`py-6 px-8 text-4xl rounded-lg shadow-lg shadow-slate-950/50 dark:shadow-slate-500/15 outline-none bg-neutral-800 placeholder:text-neutral-600 max-w-full w-[800px] border-2 border-transparent transition-all duration-75 h-[92px] ${openSans.className}`, {
